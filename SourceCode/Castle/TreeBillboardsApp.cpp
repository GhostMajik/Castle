...
void TreeBillboardsApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.Lights[2].Strength = { 0.5f, 0.5f, 0.0f };

	//front center of castle light
	...
	//center of castle light
	mMainPassCB.Lights[5].Strength = { 0.0f, 0.0f, 1.0f };
	mMainPassCB.Lights[5].Position = { 0.0f, 10.0f, -10.0f };
	mMainPassCB.Lights[5].FalloffStart = 5.0f;
	


	//ad more lights here, update HLSL
...
//update descriptor heaps with new textures added
void TreeBillboardsApp::BuildDescriptorHeaps()
{
	...
	//update number of Descriptor heaps when adding in new textures
...
void TreeBillboardsApp::BuildTreeSpritesGeometry()
{
	...
	for(UINT i = 0; i < treeCount; ++i)
	{
		...
		float z = MathHelper::RandF(-40.0f, 40.0f);
		float y = 0;
		if (x > -12.0f && x < 12.0f) {
			float sideRandomizer = MathHelper::RandF(1.0f, 100.0f);
			if (sideRandomizer >= 50)
				x = MathHelper::RandF(12.0f, 40.0f);
			else
				x = MathHelper::RandF(-40.0f, -12.0f);
		}
